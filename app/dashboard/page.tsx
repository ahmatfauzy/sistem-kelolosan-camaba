...
import { SimpleStatsCard } from "@/components/ui/simple-stats-card"
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
import { exportToExcel, importFromExcel, exportTemplateExcel } from "@/lib/excel-utils"
import {
  Upload,
  Plus,
  Save,
...
export default function DashboardPage() {
  ...
  const [editingMahasiswa, setEditingMahasiswa] = useState<CalonMahasiswaWithScores | null>(null)
  ...
  const handleNilaiChange = (kriteriaId: string, nilai: string) => {
    ...
    setNewMahasiswa((prev) => ({
      ...prev,
      penilaian: prev.penilaian.map((p) => (p.id_kriteria === kriteriaId ? { ...p, nilai: numericValue } : p)),
    }))
  }

  const fetchPeriodes = useCallback(async () => {
    try {
      const { data, error } = await supabase.from("periode").select("*").order("periode", { ascending: false })
  ...
  const fetchRanking = useCallback(async () => {
    ...
    try {
      const { data, error } = await supabase.from("v_topsis_hasil_akhir").select("*").order("peringkat")
  ...
  const createOrUpdateKriteria = async () => {
    ...
    try {
      ...
      if (editingKriteria) {
        ...
        const { error: insertError } = await supabase.from("kriteria").insert([kriteriaData])
        error = insertError
      }

      if (error) throw error

      toast.success(`Kriteria berhasil ${editingKriteria ? "diupdate" : "dibuat"}`)
      ...
      toast.error(`Gagal ${editingKriteria ? "mengupdate" : "membuat"} kriteria`)
    } finally {
      setLoading(false)
    }
  }
  ...
  const deleteKriteria = async (id: string) => {
    ...
    setLoading(true)
    try {
      const { error } = await supabase.from("kriteria").delete().eq("id_kriteria", id)
  ...
  const createOrUpdateMahasiswa = async () => {
    ...
    const invalidPenilaian = newMahasiswa.penilaian.some((p) => isNaN(p.nilai) || p.nilai < 0)
    if (invalidPenilaian) {
    ...
    try {
      ...
      if (editingMahasiswa) {
        ...
        const { error: deleteError } = await supabase.from("penilaian").delete().eq("id_calon", id_calon)
      ...
      const { error: penilaianError } = await supabase.from("penilaian").insert(penilaianToInsert)

      if (penilaianError) throw penilaianError

      toast.success(editingMahasiswa ? "Mahasiswa berhasil diperbarui" : "Mahasiswa berhasil ditambahkan")
  ...
  const deleteMahasiswa = async (id: string) => {
    ...
    setLoading(true)
    try {
      const { error } = await supabase.from("calon_mahasiswa").delete().eq("id_calon", id)
  ...
  const handleEditMahasiswa = (m: CalonMahasiswaWithScores) => {
    ...
    setNewMahasiswa({
      ...
      alamat: m.alamat || "",
      penilaian: kriteria.map((k) => {
        const nilai = m.penilaian.find((p) => p.id_kriteria === k.id_kriteria)?.nilai || 0
        return {
          id_kriteria: k.id_kriteria,
          nilai,
        }
  ...
  const handleImportExcel = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      setLoading(true)
      const { mahasiswa: mahasiswaData, penilaian: penilaianData } = await importFromExcel(file, kriteria)

      const { data: insertedMahasiswa, error: mahasiswaError } = await supabase
        .from("calon_mahasiswa")
        .insert(mahasiswaData.map((m) => ({ ...m, id_periode: selectedPeriode })))
        .select()
      ...
      for (const mhs of insertedMahasiswa) {
        for (const penilaianItem of penilaianData) {
          if (penilaianItem.nama_mahasiswa === mhs.nama) {
            const kriteriaMatch = kriteria.find((k) => k.nama === penilaianItem.nama_kriteria)
            if (kriteriaMatch) {
      ...
      const { error: penilaianError } = await supabase.from("penilaian").insert(penilaianInserts)
  ...
  const handleExportTemplate = () => {
    ...
    exportTemplateExcel(kriteria, `template-${currentPeriode?.periode || "data"}`)
  ...
  const avgScore = ranking.length > 0 ? ranking.reduce((sum, r) => sum + r.nilai_preferensi, 0) / ranking.length : 0
    ...
    <div className="min-h-screen bg-background">
      <div className="container mx-auto px-8 py-8 max-w-7xl">
        ...
        <div className="text-center mb-12">
          <h1 className="text-4xl font-bold text-foreground mb-4">Dashboard SPK Kelolosan Camaba</h1>
          <p className="text-muted-foreground text-lg max-w-2xl mx-auto">
            Sistem Pendukung Keputusan menggunakan metode TOPSIS untuk menentukan ranking calon mahasiswa
          </p>
        </div>
        ...
        <Tabs defaultValue="overview" className="space-y-8">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <CardContent>
                  <div className="space-y-4">
                    {ranking.slice(0, 5).map((r, index) => (
                      <div key={r.nama} className="flex items-center justify-between p-4 border rounded-lg">
                        ...
                        <Badge variant="outline">{r.nilai_preferensi.toFixed(4)}</Badge>
                      </div>
                    ))}
                  </div>
                </CardContent>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="space-y-4">
                    {kriteria.map((k) => (
                      <div key={k.id_kriteria} className="flex items-center justify-between p-4 border rounded-lg">
                        <div className="flex items-center gap-3">
                          <span className="font-medium">{k.nama}</span>
                          <Badge variant={k.sifat === "benefit" ? "default" : "secondary"} className="text-xs">
                            {k.sifat}
                          </Badge>
                        </div>
                        <div className="text-right">
                          <div className="font-bold text-lg">{k.bobot}</div>
                          <div className="text-xs text-muted-foreground">
                            {totalBobot > 0 ? ((k.bobot / totalBobot) * 100).toFixed(1) : 0}%
                          </div>
                        </div>
                      </div>
                    ))}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Pilih periode untuk melihat dan mengedit data</CardDescription>
                </CardHeader>
                <CardContent>
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Tambah periode baru untuk evaluasi</CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  ...
                  <Button onClick={createPeriode} disabled={loading} className="w-full">
          ...
          <TabsContent value="kriteria" className="space-y-8">
            <Card>
              <CardHeader>
                <CardTitle>Kelola Kriteria</CardTitle>
                <CardDescription>
                  {selectedPeriode ? `Periode: ${currentPeriode?.periode}` : "Pilih periode terlebih dahulu"}
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="flex justify-between items-center mb-6">
                  <BobotInfo totalBobot={totalBobot} jumlahKriteria={kriteria.length} />
                  <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                    ...
                    <DialogContent>
                      <DialogHeader>
                        <DialogTitle>{editingKriteria ? "Edit Kriteria" : "Tambah Kriteria Baru"}</DialogTitle>
                        <DialogDescription>Isi detail kriteria penilaian</DialogDescription>
                      </DialogHeader>
                      <div className="space-y-4">
                        <div>
                        ...
                        <div>
                          ...
                          >
                            ...
                            <SelectContent>
                              <SelectItem value="benefit">{SIFAT_KRITERIA.benefit}</SelectItem>
                              <SelectItem value="cost">{SIFAT_KRITERIA.cost}</SelectItem>
                            </SelectContent>
                          </Select>
                        </div>
                        <Button onClick={createOrUpdateKriteria} disabled={loading} className="w-full">
                    ...
                    <TableBody>
                      {kriteria.map((k) => (
                        <TableRow key={k.id_kriteria}>
                          ...
                          <TableCell>
                            <Badge variant={k.sifat === "benefit" ? "default" : "secondary"}>{k.sifat}</Badge>
                          </TableCell>
                          <TableCell>
                            <div className="flex gap-2">
                              ...
                              <Button size="sm" variant="destructive" onClick={() => deleteKriteria(k.id_kriteria)}>
          ...
          <TabsContent value="mahasiswa" className="space-y-8">
            <Card>
              <CardHeader>
                <CardTitle>Data Mahasiswa</CardTitle>
                <CardDescription>Kelola data calon mahasiswa dan penilaian</CardDescription>
              </CardHeader>
              <CardContent className="space-y-6">
                <div className="flex gap-4 items-center flex-wrap">
                  <Dialog open={isDialogMahasiswaOpen} onOpenChange={setIsDialogMahasiswaOpen}>
                    <DialogTrigger asChild>
                    ...
                    <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
                      <DialogHeader>
                        <DialogTitle>{editingMahasiswa ? "Edit Mahasiswa" : "Tambah Mahasiswa"}</DialogTitle>
                        <DialogDescription>
                          Masukkan data lengkap mahasiswa dan nilai untuk setiap kriteria
                        </DialogDescription>
                      </DialogHeader>
                      <div className="space-y-4">
                            ...
                            {kriteria.map((k) => (
                              <div key={k.id_kriteria} className="space-y-1">
                                ...
                                <Input
                                  ...
                                  min="0"
                                  value={
                                    newMahasiswa.penilaian.find((p) => p.id_kriteria === k.id_kriteria)?.nilai || 0
                                  }
                                  onChange={(e) => handleNilaiChange(k.id_kriteria, e.target.value)}
                        ...
                        <Button onClick={createOrUpdateMahasiswa} disabled={loading} className="w-full">
                          <Save className="w-4 h-4 mr-2" />
                    ...
                    <TableBody>
                      {mahasiswa.map((m) => (
                        <TableRow key={m.id_calon}>
                          ...
                          {kriteria.map((k) => {
                            const nilai = m.penilaian.find((p) => p.id_kriteria === k.id_kriteria)?.nilai
                            return (
                              <TableCell key={k.id_kriteria} className="text-center">
                          ...
                          <TableCell>
                            <div className="flex gap-2">
                              <Button size="sm" variant="outline" onClick={() => handleEditMahasiswa(m)}>
                                <Edit className="w-4 h-4" />
                              </Button>
                              <Button size="sm" variant="destructive" onClick={() => deleteMahasiswa(m.id_calon)}>
                                <Trash2 className="w-4 h-4" />
                              </Button>
                            </div>
                          </TableCell>
                ...
                {mahasiswa.length === 0 && (
                  <div className="text-center py-12 text-muted-foreground">
                    Belum ada data mahasiswa. Tambah atau impor data untuk memulai.
                  </div>
                )}
              </CardContent>
            </Card>
          ...
          <TabsContent value="ranking" className="space-y-8">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Hasil perhitungan ranking menggunakan metode TOPSIS</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="flex justify-between items-center mb-6">
                  <div className="text-sm text-muted-foreground">Total Kandidat: {ranking.length}</div>
                      ...
                      {ranking.map((r) => (
                        <TableRow key={r.nama}>
                          <TableCell className="text-center font-bold">
                            <Badge variant={r.peringkat <= 10 ? "default" : "secondary"}>#{r.peringkat}</Badge>
                          </TableCell>
                          <TableCell className="font-medium">{r.nama}</TableCell>
                          <TableCell className="text-center">{r.nilai_preferensi.toFixed(6)}</TableCell>
                          <TableCell className="text-center">
                            <Badge variant={r.peringkat <= 10 ? "default" : "destructive"}>
                ...
                {ranking.length === 0 && (
                  <div className="text-center py-12 text-muted-foreground">
                    Belum ada data ranking. Pastikan ada data mahasiswa dan kriteria yang valid untuk menghitung
                    ranking.
                  </div>
                )}
              </CardContent>
            ...
            {ranking.length > 0 && (
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <Card>
                  <CardHeader>
                    <CardTitle>Statistik Kelulusan</CardTitle>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-4">
                      <div className="text-center">
                        <div className="text-3xl font-bold text-green-600">{lolosCount}</div>
                        <div className="text-sm text-muted-foreground">Mahasiswa Lolos</div>
                      </div>
                      <div className="text-center">
                        <div className="text-3xl font-bold text-red-600">{tidakLolosCount}</div>
                        <div className="text-sm text-muted-foreground">Tidak Lolos</div>
                      </div>
                    </div>
                  </CardContent>
                </Card>
                    ...
                    <div className="space-y-3">
                      <div className="flex justify-between">
                        ...
                        <span className="font-bold">
                          {ranking.length > 0 ? ranking[0].nilai_preferensi.toFixed(4) : "-"}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-sm">Nilai Terendah</span>
                        <span className="font-bold">
                          {ranking.length > 0 ? ranking[ranking.length - 1].nilai_preferensi.toFixed(4) : "-"}
                        </span>
                      </div>
                      <div className="flex justify-between">
                ...
                <Card>
                  <CardHeader>
                    <CardTitle>Info Periode</CardTitle>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-3">
                      <div className="flex justify-between">
                        <span className="text-sm">Periode Aktif</span>
                        <span className="font-bold">{currentPeriode?.periode || "-"}</span>
                      </div>
                      <div className="flex justify-between">
